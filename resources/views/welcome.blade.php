...
@section('content')
    <section class="hero">
        ...
        <p>Create an account, make a website project, upload your files as a ZIP, and publish it to your own secure subdomain.</p>
    ...
    <section style="margin-top:48px"><div class="eyebrow">Plans</div><h2>Start small and grow when you need to.</h2>
        <div class="grid">
            @forelse($plans as $plan)
                <article class="card"><h3>{{ $plan->name }}</h3><div class="price">{{ $plan->formattedPrice() }}<span class="muted small"> / {{ $plan->isFree() ? $plan->trial_days : $plan->access_days }} days</span></div><p class="muted">{{ $plan->description }}</p>
                    ...
                    @if($plan->trial_days)<p class="muted small">Includes a {{ $plan->trial_days }}-day trial.</p>@endif
                </article>
            @empty
