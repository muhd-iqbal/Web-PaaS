...
<div class="card form-card"><div class="eyebrow">Get started</div><h2>Create your hosting account</h2>
    <form method="POST" action="{{ route('register') }}">@csrf
        ...
        <div class="field"><label for="plan_id">Hosting plan</label><select id="plan_id" name="plan_id" required><option value="">Choose a plan</option>@foreach($plans as $plan)<option value="{{ $plan->id }}" @selected(old('plan_id') == $plan->id)>{{ $plan->name }} — ${{ number_format((float) $plan->monthly_price, 2) }}/month</option>@endforeach</select>@error('plan_id')<div class="error">{{ $message }}</div>@enderror</div>
