...
<div class="card">
    ...
    @else<div class="table-wrap"><table><thead><tr><th>Website</th><th>Runtime</th><th>Status</th><th>Created</th><th></th></tr></thead><tbody>@foreach($projects as $project)<tr><td><strong>{{ $project->name }}</strong><div class="muted small">{{ $project->slug }}</div></td><td>{{ $project->runtime->label() }}</td><td><span class="badge">{{ $project->status->label() }}</span></td><td>{{ $project->created_at->format('M j, Y') }}</td><td><a href="{{ route('projects.show', $project) }}">Manage</a></td></tr>@endforeach</tbody></table></div><div style="margin-top:20px">{{ $projects->links() }}</div>@endif
</div>
@endsection
